...
exports.lovetestCmd = async function lovetestCmd(user,guild,interaction,member) {
    ...
    const userAvatar = await jimp.read(user.displayAvatarURL({ format: 'png', size: 512 }))
    const memberAvatar = await jimp.read(member.displayAvatarURL({ format: 'png', size: 512 }))
